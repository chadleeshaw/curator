...
from core.parsers.models import ParsedMetadata, ParsedFilename, ParsedFilepath, ParsedSearchResult, ParsedDownloadFile


def test_parsed_metadata_initialization():
    """Test ParsedMetadata dataclass initialization"""
    metadata = ParsedMetadata(title="Time Magazine", issue_number=123, year=2026, month=1)
...
def test_parsed_metadata_optional_fields():
    ...
    metadata = ParsedMetadata(title="National Geographic", volume=5, language="English", country="US")
...
def test_parsed_filename_initialization():
    ...
    parsed = ParsedFilename(title="Magazine", issue_number=1, year=2026)
...
def test_parsed_filepath_initialization():
    ...
    parsed = ParsedFilepath(title_from_path="Time Magazine", language_from_path="English", year_from_path=2026)
...
def test_parsed_metadata_with_date():
    ...
    metadata = ParsedMetadata(title="Test Magazine", publication_date=date)
...
def test_parsed_filename_without_extension():
    ...
    parsed = ParsedFilename(title="Magazine")
