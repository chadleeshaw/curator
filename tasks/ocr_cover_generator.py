...
class OCRCoverGenerator:
    ...
    def __init__(
        ...
        """
        Initialize OCR cover generator.
        ...
        try:
            ...
            try:
                ...
                for mag in magazines_with_pdfs:
                    ...
                    existing_job = (
                        db_session.query(OCRJob)
                        .filter(
                            OCRJob.magazine_id == mag.id,
                            OCRJob.status.in_(
                                [
                                    OCRJob.StatusEnum.PENDING,
                                    OCRJob.StatusEnum.PROCESSING,
                                ]
                            ),
                        )
                        .first()
                    ...
                    if existing_job:
                        logger.debug(
                            f"Skipping {mag.title}: OCR job already exists (status: {existing_job.status.value})"
                        )
                        continue
