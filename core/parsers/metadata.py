    ...
    def extract_from_filename(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        if match:
            ...
            try:
                ...
                logger.warning(f"Could not parse date from filename pattern1b: {filename}")
        ...
        if match:
            ...
            suffix_clean = re.sub(
                r"\b(?:special[\.\s]+edition|hybrid|magazine|digital|print)\b", "", suffix, flags=re.IGNORECASE
            ).strip()
        ...
        if match:
            ...
            title_clean = re.sub(
                r"\b(?:quarterly|monthly|weekly|magazine|the|hacker)\b", " ", title_clean, flags=re.IGNORECASE
            )

            # Clean suffix if present
            suffix_clean = (
                re.sub(
                    r"\b(?:hybrid|magazine|digital|print|quarterly|monthly)\b", "", suffix, flags=re.IGNORECASE
                ).strip()
                if suffix
                else ""
            )
            ...
            season_months = {"spring": "03", "summer": "06", "fall": "09", "autumn": "09", "winter": "12"}
