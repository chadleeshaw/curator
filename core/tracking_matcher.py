...
class TrackingMatcher:
    ...
    def calculate_title_score(
        ...
        if token_score >= FUZZY_THRESHOLD_MEDIUM:
            return (
                ...
                f"token_sort_medium ({token_score})"
            )
    ...
    def find_best_match(
        ...
        logger.debug(
            ...
            parsed_title, len(tracking_records)
        )
        ...
        for tracking in tracking_records:
            ...
            logger.debug(
                ...
                tracking.title, tracking.id, score, breakdown
            )
        ...
        if best_match and best_match.is_match:
            logger.info(
                "Found tracking match: '%s' -> '%s' "
                "(ID: %d, score: %d, breakdown: %s)",
                parsed_title, best_match.tracking_title,
                best_match.tracking_id, best_match.score, best_match.breakdown
            )
            return best_match
        if best_match:
            logger.info(
                "Best match below threshold: '%s' -> '%s' "
                "(ID: %d, score: %d/%d, breakdown: %s)",
                parsed_title, best_match.tracking_title,
                best_match.tracking_id, best_match.score,
                self.min_score, best_match.breakdown
            )
        else:
