...
async def search_periodical_metadata(query: str = Query(...)) -> Dict[str, Any]:
    ...
    try:
        ...
        if not _metadata_providers:
            ...
            return {"found": False, "message": "No metadata providers available", "results": []}

        all_results = []
        for provider in _metadata_providers:
            try:
                logger.debug(f"Searching metadata provider: {provider.name} (type: {provider.type})")
                provider_results = provider.search(query.strip())
                logger.debug(f"Provider {provider.name} returned {len(provider_results)} results")
                all_results.extend(provider_results)
            except Exception as e:
                logger.warning(f"Error searching metadata provider {provider.__class__.__name__}: {e}", exc_info=True)
        ...
        if all_results:
            ...
            return {"found": False, "message": f"No metadata found for '{query}'", "results": []}
