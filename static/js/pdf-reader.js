...
class PDFReader {
  ...
  /**
   * Load pdf metadata including page list
   */
  async loadMetadata() {
    try {
      ...
      document.getElementById('pdf-title').textContent = this.metadata.title || 'PDF Reader';
      ...
      this.showError('Failed to load pdf metadata: ' + error.message);
    }
  }
    ...
    pageList.innerHTML = this.metadata.pages
      .map(
        ...
        <div 
          ...
          data-index="${index}"
          onclick="pdfReader.loadPage(${index})"
        >
          <img 
            src="/api/periodicals/${this.magazineId}/pdf/page/${index}/thumbnail" 
            alt="Page ${index + 1}"
  ...
  async loadSpreadPages(index) {
    const contentDiv = document.getElementById('page-content');
    const imageUrl1 = `/api/periodicals/${this.magazineId}/pdf/page/${index}`;
    const imageUrl2 = `/api/periodicals/${this.magazineId}/pdf/page/${index + 1}`;
  ...
  setupFullscreenListeners() {
    ...
    if (fullscreenBtn) {
      ...
      const toggleHandler = (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('[PDFReader] Fullscreen button clicked/touched');
      ...
      console.log('[PDFReader] Fullscreen button event listeners attached');
    } else {
      console.warn('[PDFReader] Fullscreen button not found in DOM');
    }
